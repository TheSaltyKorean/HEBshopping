...
| W8.1 screen support | ✅ Echo Show renders the list on launch, on read, and best-effort after a write |
| W10 deploy | ✅ Terraform, DynamoDB store — see `docs/deploy.md`; **applied and running**, verified end to end from a real Echo. The MCP HTTP endpoint (off by default, `enable_mcp_url`) is a separate surface not exercised by that test |
